...
import {
	...
	Button,
	Text,
} from '@ssword-ui/react'; // assume @ssword-ui/react
import {
	Bold as BoldIcon,
...
// import your local react versions of these sections
import PostHeroSection from '@/app/';
...
// simple scroll helper (ported from @ssword/utils.scroll)
...
export default function Home(): JSX.Element {
	...
	// theme state fallback (if your app provides a theme context, replace this)
	const [themeMode, setThemeMode] = useState<'light' | 'dark'>('light');

	// handlers
	const toggle = (setter: (v: boolean) => void) => () => setter((s) => !s);
			...
			<section className="min-h-screen flex flex-col items-center justify-center bg-gradient-to-br from-background via-muted/20 to-background text-text transition-all duration-500 px-4">
				<div className="max-w-4xl text-center space-y-8">
					...
					<div className="flex flex-col sm:flex-row justify-center gap-4 pt-6">
						...
						>
							<LightbulbIcon
								className={`w-5 h-5 mr-2 ${themeMode === 'dark' ? 'text-yellow-400' : 'text-blue-400'}`}
							/>
							...
							>
								{premiumFeature ? (
									<>
										...
										ssword-ui components work together to create rich, interactive interfaces with
										smooth transitions and cohesive design language.
									</>
								) : (
									<>
			...
			<section className="py-20 bg-muted/20">
				<div className="max-w-6xl mx-auto px-4">
					<div className="text-center mb-16">
						<h2 className="text-3xl sm:text-4xl font-bold text-text mb-4">Why Choose ssword-ui?</h2>
